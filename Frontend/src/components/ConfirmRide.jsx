import React from 'react'
import { Link } from 'react-router-dom'

const ConfirmRide = (props) => {
  return (
    <div className="flex flex-col px-5 mt-2 gap-5">
      <h1
        className="text-2xl text-center 
     text-green-600
     font-semibold mt-5"
      >
        ✨Confirm your ride ✨
      </h1>

      <div className="flex flex-col justify-center gap-2 max-w-2xl border-2 border-black py-3 px-4 rounded-2xl">
        <div className="flex justify-center">
          <img
            className="h-30 w-70"
            src="data:image/jpeg;base64,"
            alt=""
          />
        </div>
        <div>
          <p
            className="text-xl 
          font-bold
        text-green-600
        text-center"
          >
            Pickup location
          </p>
          <p className="text-sm text-center font-bold mt-5">{props.pickup}</p>
        </div>

        <div>
          <p
            className="text-xl text-center 
       font-bold
      text-green-600
      "
          >
            Destination
          </p>
          <p className="text-sm text-center font-bold mt-5">
            {props.destination}
          </p>
        </div>

        <div>
          <p className="text-xl text-green-600 text-center  font-bold   ">
            You have to pay
          </p>
          <p className="text-xl bg-amber-300 rounded-b-full text-center font-bold mt-5">
            ₹{props.fare[props.vehicleType]}
          </p>
        </div>
      </div>
      <div className='flex justify-center'>
        <button
          onClick={() => {
            props.createRide();
            props.setDriver(true);
            props.setConfirmride(false);
          }}
          className="bg-green-500 text-center w-43 py-1 rounded-xl"
        >
          Confirm Ride
        </button>
      </div>
    </div>
  );
}

export default ConfirmRide
