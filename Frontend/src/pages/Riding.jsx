...
const Riding = () => {
  ...
  const initPay = (order) => {
    const options = {
      ...
      handler: async function (response) {
        ...
        if (data.status == 200) {
          ...
          }/success?reference=${response.razorpay_payment_id}`;
        } else {
          window.location.href = `${
            import.meta.env.VITE_FRONTEND_URL
    ...
    const rzp = new window.Razorpay(options); // Razorpay(options);
    rzp.open();
  };
  ...
  const handlePayment = async (fare) => {
    const responseRazorpay = await axios.post(
      ...
      {
        amount: fare,
        rideId:ride._id // changed from 'fare' to 'amount'
      },
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  return (
    <div className="h-screen ">
      ...
      <div
        className=" flex flex-col mt-3 container mx-auto gap-4 max-w-2xl items-center
